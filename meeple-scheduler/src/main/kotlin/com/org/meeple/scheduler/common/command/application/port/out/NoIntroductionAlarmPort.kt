 ...
 * 배치가 하루에 여러 번 돌아도 중복 알림이 가지 않도록, 구현은 [now]가 속한 당일 이미 같은 알림을 받은 수신자를 건너뛴다.
 */
interface NoIntroductionAlarmPort {

	/** 솔로 매칭에서 오늘 소개받지 못한 [userIds] 각자에게 알람을 보낸다. ([now] 당일 이미 받은 유저는 제외) */
	fun notifySoloUnmatched(userIds: Collection<Long>, now: LocalDateTime)

	/** 팀 매칭에서 오늘 소개받지 못한 [teamIds] 각 팀의 활성 구성원 전원에게 알람을 보낸다. ([now] 당일 이미 받은 구성원은 제외) */
	fun notifyTeamUnmatched(teamIds: Collection<Long>, now: LocalDateTime)
}
