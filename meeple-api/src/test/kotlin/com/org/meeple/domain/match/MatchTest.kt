 ...
 * 시작 매칭은 core testFixtures의 [MatchFixture]로 만든다.
 */
class MatchTest : DescribeSpec({
	...
	describe("respond - 성사 시 만료 연장") {
		it("양쪽이 수락해 MATCHED가 되면 만료 시각을 100년 뒤로 미룬다") {
			val proposed: Match = MatchFixture.create(maleUserId = maleUserId, femaleUserId = femaleUserId)
	...
	describe("MatchAccepted.from") {
		it("성사된 매칭의 id·남녀 식별자와 수락자를 이벤트로 옮긴다") {
			...
			event shouldBe MatchAccepted(
				matchId = 7L,
				maleUserId = maleUserId,
				femaleUserId = femaleUserId,
				acceptedByUserId = maleUserId,
			)
			// 수락자(남)의 상대(= 알람 수신자)는 여성이다.
			event.partnerOfAcceptor shouldBe femaleUserId
		}
	}

	describe("InterestSent.from") {
		it("보낸 사람의 상대를 수신자로 하는 이벤트를 만든다") {
			val match: Match = MatchFixture.create(id = 7L, maleUserId = maleUserId, femaleUserId = femaleUserId)
