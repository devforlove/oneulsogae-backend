...
/**
 * [com.org.meeple.core.popup.domain.Popup] 도메인 유닛 테스트.
...
class PopupTest : DescribeSpec({
	...
	describe("isVisible") {
		it("노출 ON이고 기간 제한이 없으면 노출 대상이다") {
			PopupFixture.create().isVisible(now) shouldBe true
		}

		it("노출 OFF면 기간과 무관하게 노출 대상이 아니다") {
			PopupFixture.create(exposed = false).isVisible(now) shouldBe false
		}
