...
/**
 * 회사 이메일 인증(직장 인증) 요청을 추적하는 영속성 엔티티.
 * 발송한 1회용 인증번호(code)와 만료/검증 시각을 보관한다.
 * 도메인 로직은 [com.org.oneulsogae.core.user.domain.CompanyEmailVerification] 모델에 정의한다.
 */
@Entity
@SQLRestriction("deleted_at is null")
@Table(
	name = "company_email_verifications",
	indexes = [
		// user_id로 필터 후 PK(id) 내림차순으로 최신 1건을 찾는다. (InnoDB 보조 인덱스는 PK를 포함)
		Index(name = "idx_user_id", columnList = "user_id"),
	],
)
class CompanyEmailVerificationEntity(
