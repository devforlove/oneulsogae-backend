 ...
 * 나가려는 사용자의 활성 참가자([com.org.meeple.core.chat.command.domain.ChatRoomMember]) 행을 로드해(없거나 이미 나갔으면 비참가자로 보고 거절) 나가기를 처리한다.
 * 실제 비활성화·남는 상대에게 나감 안내·마지막 한 명이 나가면 방 종료(소프트 삭제)는 매칭 종료·팀 해체와 동일한 [DeactivateChatRoomMemberUseCase]에 위임해 재사용한다.
...
class LeaveChatRoomService(
	...
	override fun leave(userId: Long, chatRoomId: Long) {
		...
		// 본인 비활성화 + 남는 상대 알림 + 마지막이면 방 종료까지: 매칭 종료·팀 해체와 동일한 비활성화 로직을 재사용한다. (방 종류/식별자는 방에서 얻는다)
		deactivateChatRoomMemberUseCase.deactivate(chatRoom.matchType, chatRoom.matchId, listOf(userId))
	}
}
