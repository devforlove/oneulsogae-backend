package com.org.meeple.core.chat.query.service
...
import com.org.meeple.core.chat.query.dao.ChatMessageQueryDao
 ...
 * 접근 검증은 매 요청마다 한다. 첫 페이지는 어차피 가져온 참가자 목록([ChatParticipantQueryDao.findByChatRoomId])으로 검증하고,
 * 커서 페이지는 프로필 조인 없이 단건 존재([ChatRoomMemberQueryDao.existsByChatRoomIdAndUserId])로 가볍게 검증한다. (커서만 바꿔 비참가자가 페이징하는 것을 막는다)
...
class GetChatRoomDetailService(
	...
	private val chatParticipantQueryDao: ChatParticipantQueryDao,
	private val chatRoomMemberQueryDao: ChatRoomMemberQueryDao,
	private val chatMessageQueryDao: ChatMessageQueryDao,
	...
	private fun firstPage(userId: Long, chatRoomId: Long, pageSize: Int): ChatRoomDetail {
		val chatRoom: ChatRoomView = chatRoomQueryDao.findById(chatRoomId)
			?: throw BusinessException(ChatErrorCode.CHAT_ROOM_NOT_FOUND)
		val participants: ChatParticipants = chatParticipantQueryDao.findByChatRoomId(chatRoomId)
		participants.validateParticipant(userId)

		val messages: ChatMessageViews = chatMessageQueryDao.findByChatRoom(chatRoomId, null, pageSize)
	...
	private fun nextPage(userId: Long, chatRoomId: Long, beforeMessageId: Long, pageSize: Int): ChatRoomDetail {
		if (!chatRoomMemberQueryDao.existsByChatRoomIdAndUserId(chatRoomId, userId)) {
			throw BusinessException(ChatErrorCode.NOT_CHAT_ROOM_PARTICIPANT)
		}
		val messages: ChatMessageViews = chatMessageQueryDao.findByChatRoom(chatRoomId, beforeMessageId, pageSize)
		return ChatRoomDetail(
