package com.org.meeple.core.solomatch.command.domain.event
 ...
 * [acceptedByUserId]는 마지막에 수락해 성사를 만든 사용자, [partnerOfAcceptor]는 그 상대다.
...
data class MatchAccepted(
	...
	companion object {
		...
		/** 성사된 매칭과 수락자로부터 이벤트를 만든다. 상대는 매칭 참가자에서 파생한다. (status MATCHED 전제 검증은 호출 측 책임) */
		fun from(match: Match, acceptedByUserId: Long): MatchAccepted =
			MatchAccepted(
				matchId = match.id,
				acceptedByUserId = acceptedByUserId,
				partnerOfAcceptor = match.partnerOf(acceptedByUserId),
			)
	}
}
