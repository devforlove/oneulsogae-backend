...
/**
 * 이미지 공개 프록시. key로 새 presigned GET URL을 발급해 302 리다이렉트한다(비공개 버킷 유지, 만료 무관).
 * gathering-descriptions/(모임 소개)·popups/(팝업) 프리픽스 key만 허용 — 그 외는 404.
 * 비로그인 소비자도 이미지를 봐야 하므로 공개(permitAll).
 */
@Tag(name = "이미지 프록시", description = "모임 소개·팝업 이미지를 presigned URL로 302 리다이렉트하는 공개 엔드포인트.")
@RestController
class GatheringDescriptionImageController(
	private val useCase: GetGatheringDescriptionImageUrlUseCase,
	private val getPopupImageUrlUseCase: GetPopupImageUrlUseCase,
) {

	@Operation(
		summary = "이미지 조회(302 리다이렉트)",
		description = "gathering-descriptions/·popups/ 프리픽스 key만 허용한다. 유효하면 presigned GET URL로 302 리다이렉트, " +
	...
	fun image(@PathVariable key: String): ResponseEntity<Void> {
		...
		val url = useCase.execute(normalized)
			?: getPopupImageUrlUseCase.execute(normalized)
