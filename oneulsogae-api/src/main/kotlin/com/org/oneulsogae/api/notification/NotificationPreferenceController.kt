 ...
 * - GET /: 현재 로그인 사용자의 알림 설정 7개를 조회한다(없으면 기본값).
 * - PUT /: 7개 전체를 교체한다(full replace, 멱등 upsert).
 */
...
class NotificationPreferenceController(
	...
	@Operation(summary = "내 알림 설정 조회", description = "현재 로그인 사용자의 알림 설정 7개를 조회한다. 설정한 적 없으면 기본값을 반환한다.")
	@GetMapping
	fun myPreference(
	...
	@Operation(summary = "내 알림 설정 저장", description = "현재 로그인 사용자의 알림 설정 7개를 전체 교체한다. (멱등 upsert)")
	@PutMapping
