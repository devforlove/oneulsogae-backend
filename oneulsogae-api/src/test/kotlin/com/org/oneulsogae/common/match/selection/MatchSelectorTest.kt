...
class MatchSelectorTest : DescribeSpec({
	...
	describe("selectBest - 같은 회사 소개 차단") {
		val sameCompanyNear = near.copy(companyName = "오늘의 소개컴퍼니", refuseSameCompanyIntro = false)
		...
		it("대상이 거부하지 않아도 같은 회사 후보가 거부하면 제외된다(양방향)") {
			val refusingSameCompany = near.copy(companyName = "오늘의 소개컴퍼니", refuseSameCompanyIntro = true)
			val picked: Cand? = selectBest(
		...
		it("대상 회사가 미상(null)이면 차단 없이 최고점 후보를 고른다") {
			val picked: Cand? = selectBest(
				candidates = listOf(near.copy(companyName = "오늘의 소개컴퍼니")),
