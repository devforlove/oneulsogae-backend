...
class TeamMatchTest : DescribeSpec({
	...
	describe("respond - 팀 관심 신청/성사") {
		it("한 팀만 신청하면 그 팀이 APPLY, 매칭은 PARTIALLY_ACCEPTED가 된다") {
			...
			val responded: TeamMatch = teamMatch.respond(10L, 100L, paidInitAmount = 40)
		...
		it("양 팀이 모두 신청하면 MATCHED + 전원 ACTIVE가 되고 만료가 100년 연장된다") {
			...
			val matched: TeamMatch = teamMatch.respond(10L, 100L, paidInitAmount = 40).respond(20L, 200L, paidInitAmount = 40)
		...
		it("APPLY 전이 시 응답한 팀의 paidInitAmount에 전달받은 지불액을 스냅샷한다 (여성팀 20)") {
			val teamMatch: TeamMatch = TeamMatch.propose(10L, 20L, TeamMatchType.RECOMMENDED, now)

			val responded: TeamMatch = teamMatch.respond(10L, 100L, paidInitAmount = 20)

			responded.matchedTeams.find(10L)!!.paidInitAmount shouldBe 20
		}
	}
	...
	describe("validateTerminable - 팀 매칭 종료 검증") {
		...
		fun matched(): TeamMatch =
			TeamMatch.propose(10L, 20L, TeamMatchType.RECOMMENDED, now).respond(10L, 100L, paidInitAmount = 40).respond(20L, 200L, paidInitAmount = 40)
		...
		it("성사(MATCHED) 전이면 TEAM_MATCH_NOT_MATCHED를 던진다") {
			val partiallyAccepted: TeamMatch = TeamMatch.propose(10L, 20L, TeamMatchType.RECOMMENDED, now).respond(10L, 100L, paidInitAmount = 40)
	...
	describe("leave - 팀 매칭 종료") {
		...
		it("상대 팀이 활성이면 내 팀만 DEACTIVE로 전이되고(소프트 삭제 안 함) 헤더는 MATCHED로 유지된다") {
			...
			val ten: com.org.oneulsogae.core.teammatch.command.domain.MatchedTeam = left.matchedTeams.values.first { it.teamId == 10L }
	...
	describe("failureRefunds - 미성사 만료 환불 산정") {
		it("신청(APPLY)한 팀의 지불자에게만 신청 비용의 절반을 환불한다") {
			...
			val partiallyAccepted: TeamMatch =
				TeamMatch.propose(10L, 20L, TeamMatchType.RECOMMENDED, now).respond(10L, 100L, paidInitAmount = 40)

			val refunds: List<com.org.oneulsogae.core.teammatch.command.domain.MatchRefund> = partiallyAccepted.failureRefunds()
		...
		it("성사(MATCHED)되어 전원 ACTIVE면 환불 대상이 없다") {
			val matched: TeamMatch = TeamMatch.propose(10L, 20L, TeamMatchType.RECOMMENDED, now).respond(10L, 100L, paidInitAmount = 40).respond(20L, 200L, paidInitAmount = 40)

			matched.failureRefunds() shouldBe emptyList()
		}

		it("실차감액(paidInitAmount) 스냅샷의 절반을 환불한다 (여성팀 20 지불 → 10 환불)") {
			val partiallyAccepted: TeamMatch =
				TeamMatch.propose(10L, 20L, TeamMatchType.RECOMMENDED, now).respond(10L, 100L, paidInitAmount = 20)

			val refunds: List<com.org.oneulsogae.core.teammatch.command.domain.MatchRefund> = partiallyAccepted.failureRefunds()

			refunds.size shouldBe 1
			refunds.first().userId shouldBe 100L
			refunds.first().amount shouldBe 10
		}
	}
})
