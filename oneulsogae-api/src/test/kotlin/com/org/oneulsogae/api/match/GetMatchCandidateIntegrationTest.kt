    ...
    describe("findOneCandidate") {
        ...
        context("요청자가 같은 회사 소개를 거부하면") {
            it("같은 회사 후보는 제외하고 다른 회사 후보를 반환한다") {
                ...
                persistMatchUser(userId = 10L, gender = Gender.FEMALE, regionId = regionId, lastLoginAt = LocalDateTime.now(), companyName = "오늘의 소개컴퍼니")
                ...
                val candidateId: Long? = getMatchCandidatePort.findOneCandidate(
                    requesterId = 1L, gender = Gender.FEMALE, regionId = regionId, loginAfter = loginAfter,
                    requesterCompanyName = "오늘의 소개컴퍼니", requesterRefusesSameCompanyIntro = true,
                )
        ...
        context("요청자는 거부를 해제했지만 같은 회사 후보가 거부 중이면") {
            it("그 후보를 제외한다 (양방향 차단)") {
                val regionId: Long = persistRegion("서울특별시", "강남구", 37.50, 127.00)
                persistMatchUser(userId = 10L, gender = Gender.FEMALE, regionId = regionId, companyName = "오늘의 소개컴퍼니", refuseSameCompanyIntro = true)
                regionProximityPort.refresh()

                getMatchCandidatePort.findOneCandidate(
                    requesterId = 1L, gender = Gender.FEMALE, regionId = regionId, loginAfter = loginAfter,
                    requesterCompanyName = "오늘의 소개컴퍼니", requesterRefusesSameCompanyIntro = false,
                ).shouldBeNull()
            }
        }
        ...
        context("같은 회사라도 양쪽 모두 거부를 해제했으면") {
            it("그 후보를 반환한다") {
                ...
                val sameCompanyId: Long = persistMatchUser(
                    userId = 10L, gender = Gender.FEMALE, regionId = regionId, companyName = "오늘의 소개컴퍼니", refuseSameCompanyIntro = false,
                )
                regionProximityPort.refresh()
