...
import com.org.oneulsogae.infra.fixture.MemberVerificationEntityFixture
	...
	describe("POST /admin/v1/member-verifications/{id}/approve") {
		...
		it("승인하면 status=APPROVED로 바꾸고 회사명(user_details·match_user)·gathering_profile(직종·직장상세·생일·키)을 확정한다 (200)") {
			...
			post("/admin/v1/member-verifications/$id/approve") {
				bearer(adminAccessTokenFor(9901L))
				jsonBody("""{"companyName":"오늘의 소개","jobCategory":"IT·개발직","jobDetail":"오늘의 소개 백엔드 개발자"}""")
			} expect {
				status(200)
				body("success", true)
			}
			...
			detail.companyName shouldBe "오늘의 소개"
		...
		it("필수 입력(회사명 등)이 비면 400이다") {
			...
			post("/admin/v1/member-verifications/$id/approve") {
				bearer(adminAccessTokenFor(9901L))
				jsonBody("""{"companyName":"","jobCategory":"IT·개발직","jobDetail":"오늘의 소개 백엔드 개발자"}""")
			} expect {
				status(400)
				body("success", false)
			}
