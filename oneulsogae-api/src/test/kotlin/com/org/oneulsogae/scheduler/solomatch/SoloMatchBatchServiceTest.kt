...
class SoloMatchBatchServiceTest : DescribeSpec({
	...
	describe("run - 같은 회사 소개 차단") {
		...
		it("같은 회사이고 대상이 거부하면 그 후보를 건너뛰고 다음 후보를 소개한다") {
			...
			val matchables: List<MatchableUser> = listOf(
				male(1001L, now, companyName = "오늘의 소개컴퍼니", refuseSameCompanyIntro = true),
				female(1002L, now.minusMinutes(1), companyName = "오늘의 소개컴퍼니", refuseSameCompanyIntro = false),
		...
		it("대상이 거부하지 않아도 같은 회사 후보가 거부하면 차단된다(양방향)") {
			val matchables: List<MatchableUser> = listOf(
				male(1001L, now, companyName = "오늘의 소개컴퍼니", refuseSameCompanyIntro = false),
				female(1002L, now.minusMinutes(1), companyName = "오늘의 소개컴퍼니", refuseSameCompanyIntro = true),
			)
			val saves: MutableList<Pair<Long, Long>> = mutableListOf()
