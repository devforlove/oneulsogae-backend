...
/**
 * 학교 서류 이미지 인증 도메인 모델. (직장 서류 인증 [CompanyImageVerification]과 같은 구성)
 * 사용자가 재학·졸업증명서 등 학교 서류 이미지를 업로드하면, 그 파일의 S3 오브젝트 키([imageKey])와
 * 심사 상태([status])를 보관한다. 서류는 자동 검증이 불가능해 제출 시 [UniversityImageVerificationStatus.PENDING]으로
 * 시작하며, 어드민 심사로 승인/반려된다.
 */
data class UniversityImageVerification(
	val id: Long = 0,
	val userId: Long,
	/** 업로드한 서류 파일의 S3 오브젝트 키. (버킷은 설정에서 관리 — 공개 URL이 아니라 키만 보관해 노출을 막는다) */
	val imageKey: String,
	val status: UniversityImageVerificationStatus = UniversityImageVerificationStatus.PENDING,
	/** 유저가 제출 시 기입한 희망 학교명. (어드민 심사 근거) */
	val universityName: String? = null,
	/** 제출 시점의 유저 프로필 학교명 스냅샷. 승인으로 프로필이 덮어써져도 심사 상세에서 이전 학교명을 보여주기 위해 보관한다. */
	val previousUniversityName: String? = null,
	...
	companion object {
		...
		/** 제출 희망 학교명의 최대 길이. (어드민 승인 DTO와 동일 상한) */
		const val MAX_UNIVERSITY_NAME_LENGTH: Int = 50

		/**
		 * 신규 제출(심사 대기)을 생성한다. 희망 학교명([universityName])을 검증해 담고,
		 * 제출 시점의 프로필 학교명([previousUniversityName])을 이전 학교명으로 스냅샷한다. (프로필 미기입 시 null)
		 */
		fun create(
			userId: Long,
			imageKey: String,
			...
			return UniversityImageVerification(
				userId = userId,
				imageKey = imageKey,
		...
		/** 제출 희망 학교명 검증. 공백이거나 [MAX_UNIVERSITY_NAME_LENGTH]자를 넘으면 [UserErrorCode.INVALID_UNIVERSITY_NAME]. */
		fun validateUniversityName(universityName: String) {
			if (universityName.isBlank() || universityName.length > MAX_UNIVERSITY_NAME_LENGTH) {
				throw BusinessException(UserErrorCode.INVALID_UNIVERSITY_NAME)
			}
		}
