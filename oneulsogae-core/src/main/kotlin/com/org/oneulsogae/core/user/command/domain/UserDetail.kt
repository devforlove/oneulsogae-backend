...
/**
 * 사용자 프로필 상세 도메인 모델. [com.org.oneulsogae.core.user.command.domain.User]와 1:1로 연결되며 닉네임/프로필 이미지 및
 * 가입 과정에서 점진적으로 채워지는 프로필 정보(나이/키/성별 등)를 담는다.
 * 사용자 프로필 속성 묶음으로, user 도메인이 소유한다.
 * 영속성은 [com.org.oneulsogae.infra.user.command.entity.UserDetailEntity]가 담당한다.
 */
data class UserDetail(
	...
	companion object {
		...
		/** [com.org.oneulsogae.core.user.command.domain.User]에 연결되는 신규 프로필 상세를 생성한다. */
