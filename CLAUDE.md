...
- **메시지 형식**: `<type>(<domain>): <설명>`. 타입 옆 괄호에 변경이 속한 **도메인**(`match`·`user`·`coin`·`chat`·`alarm`·`scheduler` 등)을 표시한다. 도메인이 모호하거나 전역(빌드·CI·루트 문서 등)이면 괄호를 생략한다. 예: `fix(match): 매치 코드 수정`, `feat(alarm): 팀 초대 알람 추가`, `docs: 커밋 컨벤션 추가`.
- **타입**:
  - `feat`: 새로운 기능 추가
  - `fix`: 버그 수정
  - `docs`: 문서 수정
...
- 영속성 구현은 `oneulsogae-infra`의 Adapter가 core/scheduler/chatting의 **out-port를 구현**한다.
- HTTP 경계(Controller / 요청·응답 DTO)는 `oneulsogae-api`에만, 공용 enum/상수는 `oneulsogae-common`에 둔다.
- `oneulsogae-scheduler`(배치)·`oneulsogae-chatting`(WebSocket)은 **core에 의존하지 않는 자립 모듈**이다. infra 어댑터가 core 도메인에 위임해 잇는다.
...
- **도메인 로직 캡슐화**: 도메인 규칙(컬렉션 순회·상태 조건 판정·집계 등)을 서비스에 인라인하지 않는다. 서비스가 `members.values.none { … }`처럼 일급 컬렉션의 `values`를 들춰 조건을 직접 계산하면 안 된다. 의미 있는 이름의 도메인 모델/일급 컬렉션 메서드(예: `allInactiveAfterLeaving(...)`)로 캡슐화하고 서비스는 그 결과만 사용한다.
...
- **도메인 모델 → Kotest 유닛 테스트**(프레임워크·인프라 없이, 외부 의존은 파라미터 주입). **`oneulsogae-api` → E2E 테스트**(실서버 + Testcontainers).
