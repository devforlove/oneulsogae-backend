...
oneulsogae 서비스 초기 배포용 AWS 콘솔 셋업 가이드. **프리티어 전제**의 최소 비용 구성이며, Terraform/IaC 없이 콘솔에서 순서대로 따라 하도록 작성했다.
...
사용자
 ...
 └─ api.<도메인>  → EC2 t4g.small (퍼블릭 서브넷)
                     ├─ Caddy (HTTPS)
                     └─ oneulsogae-api 컨테이너 (WS·스케줄러 포함)
...
- [ ] **리포에 커밋된 시크릿 로테이션**: `oneulsogae-api/src/main/resources/application.yml`에 Google OAuth client-secret(`GOCSPX-...`)과 KCP enc-key가 기본값으로 하드코딩돼 있다. **각 콘솔에서 재발급**하고, 새 값은 리포에 넣지 않는다(9단계에서 SSM으로 주입).
- [ ] 도메인 준비 (가비아·Route 53 등 어디서 사든 무방. 예시는 `oneulsogae.example` 사용)
...
1. `oneulsogae-ec2-sg` (EC2용)
   - 인바운드: `80`(HTTP) `0.0.0.0/0`, `443`(HTTPS) `0.0.0.0/0`
   - SSH(22)는 **열지 않는다** (접속은 SSM Session Manager 사용, 10단계)
   - 아웃바운드: 전체 허용(기본값)
2. `oneulsogae-rds-sg` (RDS용)
   - 인바운드: `3306` — 소스에 **`oneulsogae-ec2-sg` 선택** (CIDR 아님, 보안 그룹 참조)
3. `oneulsogae-redis-sg` (Redis용)
   - 인바운드: `6379` — 소스에 **`oneulsogae-ec2-sg` 선택**
...
1. 이름: `oneulsogae-prod-uploads` (전역 유일해야 함)
...
3. 템플릿: **프리 티어**
4. 설정:
   - DB 인스턴스 식별자: `oneulsogae-prod-db`
   - 마스터 사용자 이름: `oneulsogae`
...
7. 연결:
   ...
   - VPC: 기본 VPC
   - **퍼블릭 액세스: 아니오**
   - VPC 보안 그룹: **기존 항목 선택 → `oneulsogae-rds-sg`** (기본 sg 제거)
   - 가용 영역: 아무거나
8. 추가 구성:
   - 초기 데이터베이스 이름: **`oneulsogae`** (반드시 입력. 안 넣으면 DB가 안 만들어짐)
   - 자동 백업: **활성화, 보존 7일**
   - 파라미터 그룹은 나중에 collation 이슈 시 조정 (아래 주의 참고)
9. 생성 → 엔드포인트(`oneulsogae-prod-db.xxxx.ap-northeast-2.rds.amazonaws.com`) 기록
...
2. 클러스터 모드: **비활성화**
3. 설정:
   - 이름: `oneulsogae-prod-redis`
   - 노드 유형: `cache.t4g.micro` (프리티어)
   - 복제본 수: **0** (프리티어·초기)
4. 서브넷 그룹: 새로 생성, 기본 VPC의 서브넷 2개 선택
5. 보안: VPC 보안 그룹 **`oneulsogae-redis-sg`** 선택
...
7. 생성 → 기본 엔드포인트(`oneulsogae-prod-redis.xxxx.cache.amazonaws.com:6379`) 기록

> 앱에서 Redis는 모바일 인증코드 저장·배치 분산 락(Redisson) 용도라 가용성 요구가 낮다. 프리티어 종료 후 비용($12/월)이 아까우면 EC2 내 도커 Redis로 되돌리고 `REDIS_HOST`만 바꾸면 된다.
...
3. 역할 이름: `oneulsogae-ec2-role` → 생성
...
```json
{
  ...
  "Statement": [{
    ...
    "Resource": "arn:aws:s3:::oneulsogae-prod-uploads/*"
  }, {
    "Effect": "Allow",
...
아래를 각각 **SecureString** 타입으로 생성 (이름 규칙 `/oneulsogae/prod/...`):

| 파라미터 이름 | 값 |
|---|---|
| `/oneulsogae/prod/DB_PASSWORD` | 3단계에서 만든 RDS 마스터 암호 |
| `/oneulsogae/prod/JWT_SECRET` | 64바이트 이상 랜덤 문자열 (`openssl rand -base64 64`) |
| `/oneulsogae/prod/KAKAO_CLIENT_ID` | 카카오 REST API 키 |
| `/oneulsogae/prod/KAKAO_CLIENT_SECRET` | 카카오 client secret(사용 시) |
| `/oneulsogae/prod/GOOGLE_CLIENT_ID` | **재발급한** 구글 OAuth client id |
| `/oneulsogae/prod/GOOGLE_CLIENT_SECRET` | **재발급한** 구글 client secret |
| `/oneulsogae/prod/KCP_ENC_KEY` | **재발급한** KCP enc-key |
| `/oneulsogae/prod/KCP_SITE_CD` | KCP 사이트 코드(운영) |
...
5. 네트워크 설정 → 편집:
   - VPC: 기본 VPC / 퍼블릭 서브넷
   - 퍼블릭 IP 자동 할당: **활성화**
   - 방화벽: **기존 보안 그룹 선택 → `oneulsogae-ec2-sg`**
6. 스토리지: gp3 30GB
7. 고급 세부 정보 → **IAM 인스턴스 프로파일: `oneulsogae-ec2-role`**
8. 시작
...
```bash
# SSM에서 시크릿 읽기 (예시)
DB_PASSWORD=$(aws ssm get-parameter --name /oneulsogae/prod/DB_PASSWORD --with-decryption --query Parameter.Value --output text --region ap-northeast-2)
...
# --- DB (RDS) ---
DB_HOST=oneulsogae-prod-db.xxxx.ap-northeast-2.rds.amazonaws.com
DB_NAME=oneulsogae
DB_USERNAME=oneulsogae
DB_PASSWORD=<SSM에서 읽은 값>

# --- Redis (ElastiCache) ---
REDIS_HOST=oneulsogae-prod-redis.xxxx.cache.amazonaws.com
REDIS_PORT=6379
...
# --- S3 (IAM 역할 사용 → 키 불필요) ---
S3_BUCKET=oneulsogae-prod-uploads
S3_REGION=ap-northeast-2

# --- 쿠키·CORS·리다이렉트 (운영값) ---
AUTH_COOKIE_DOMAIN=.oneulsogae.example
AUTH_COOKIE_SECURE=true
AUTH_COOKIE_SAME_SITE=Lax
CORS_ALLOWED_ORIGINS=https://app.oneulsogae.example
OAUTH2_REDIRECT_URI=https://app.oneulsogae.example/oauth/redirect
OAUTH2_FAILURE_REDIRECT_URI=https://app.oneulsogae.example
OAUTH2_ADMIN_REDIRECT_URI=https://admin.oneulsogae.example/oauth/redirect

# --- KCP (운영·재발급값) ---
KCP_SITE_CD=<SSM>
KCP_ENC_KEY=<SSM·재발급값>
KCP_BASE_URL=https://cert.kcp.co.kr
KCP_RET_URL=https://app.oneulsogae.example/api/onboarding/identity/callback
```
...
> - `oneulsogae-infra/application-prod.yml`이 DB(`DB_HOST`)·Redis(`REDIS_HOST`)를 환경변수로 받는다(위 `.env`).
...
2. IAM 역할 `oneulsogae-github-deploy-role` 생성 (신뢰: 위 OIDC, 특정 리포로 제한)
...
3. **환경변수** (Amplify 콘솔 → 호스팅 → 환경변수):
   | 변수 | 값 |
   |---|---|
   | `NEXT_PUBLIC_API_BASE_URL` | `https://api.oneulsogae.example` |
...
**Route 53 콘솔 → 호스팅 영역 생성** (`oneulsogae.example`)

1. 도메인 등록기관(가비아 등)에서 네임서버를 Route 53 NS 레코드로 변경
2. 레코드 생성:
   - `api.oneulsogae.example` → **A 레코드** → EC2 퍼블릭 IP (또는 탄력적 IP 권장)
   - `app.oneulsogae.example` → Amplify 도메인 연결 (Amplify 콘솔 → 도메인 관리에서 자동으로 CNAME/ALIAS 생성)
...
- **카카오 개발자 콘솔**: Redirect URI에 `https://api.oneulsogae.example/login/oauth2/code/kakao` 추가, 플랫폼 도메인에 `https://app.oneulsogae.example` 추가
- **구글 클라우드 콘솔**: 승인된 리디렉션 URI에 `https://api.oneulsogae.example/login/oauth2/code/google` 추가
- **카카오 JS 키**: 웹 플랫폼 도메인에 `https://app.oneulsogae.example` 등록(카톡 공유용)

---

## 14. 배포 후 점검 체크리스트

- [ ] `https://api.oneulsogae.example/swagger-ui` 가 **안 열리는지**(운영 프로파일은 springdoc off) 확인
- [ ] 헬스체크: API가 8080에서 뜨고 Caddy가 443으로 프록시하는지
- [ ] 프론트에서 로그인 → 쿠키가 `.oneulsogae.example` 도메인·Secure로 세팅되는지
