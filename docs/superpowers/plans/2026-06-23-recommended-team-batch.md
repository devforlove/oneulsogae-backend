...
**Tech Stack:** Kotlin 2.2.21 / Spring Boot 4 / Spring Data JPA + QueryDSL / Kotest(DescribeSpec) / Testcontainers(MySQL) / 헥사고날 멀티모듈(meeple-scheduler·infra·api).

## Global Constraints

- 모듈 의존 방향 준수: 배치 로직·포트·도메인·read model은 `meeple-scheduler`(core 비의존), out-port 구현(어댑터/DaoImpl)은 `meeple-infra`, Controller/Scheduler는 `meeple-api`.
...
- scheduler 도메인의 유닛/통합 테스트는 `meeple-api/src/test/...`에 둔다(scheduler 모듈에 kotest 테스트 소스셋이 없음 — 기존 `MatchPoolTest`·`RunSoloMatchBatchIntegrationTest`와 동일).
...
**meeple-scheduler** (`src/main/kotlin/com/org/meeple/scheduler/match/`)
...
**meeple-infra** (`src/main/kotlin/com/org/meeple/infra/match/query/`)
- `GetRecommendableSoloUserDaoImpl.kt` → 수정
- `GetCandidateTeamDaoImpl.kt` → 수정
- `GetRecommendedTeamRecordDaoImpl.kt` → **신규**

**meeple-api**
- `src/main/kotlin/com/org/meeple/scheduler/match/RecommendTeamBatchScheduler.kt` → 개명 `RecommendedTeamBatchScheduler.kt`
- `src/main/kotlin/com/org/meeple/api/admin/AdminRecommendTeamBatchController.kt` → 개명 `AdminRecommendedTeamBatchController.kt`
- `src/main/kotlin/com/org/meeple/api/admin/response/RecommendTeamBatchResponse.kt` → 개명 `RecommendedTeamBatchResponse.kt`
- `src/main/resources/application.yml` → cron 키 개명
- `src/test/kotlin/com/org/meeple/scheduler/match/TeamPoolTest.kt` → **신규** 유닛
- `src/test/kotlin/com/org/meeple/api/scheduler/RunRecommendedTeamBatchIntegrationTest.kt` → **신규** 통합
- `src/test/kotlin/com/org/meeple/api/admin/AdminRecommendTeamBatchE2ETest.kt` → 개명+갱신 `AdminRecommendedTeamBatchE2ETest.kt`
...
S=meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/match
...
A=meeple-api/src/main/kotlin/com/org/meeple
...
git mv meeple-api/src/test/kotlin/com/org/meeple/api/admin/AdminRecommendTeamBatchE2ETest.kt \
       meeple-api/src/test/kotlin/com/org/meeple/api/admin/AdminRecommendedTeamBatchE2ETest.kt
```
...
`meeple-api/src/main/resources/application.yml`에서 `recommend-team-batch:` 블록 키와 env 이름을 바꾼다. (들여쓰기 유지, 기본값 동일)
...
그리고 `RecommendedTeamBatchScheduler`의 `@Scheduled(cron = "\${meeple.match.recommended-team-batch.cron}", ...)`로 키를 맞춘다.
...
Run: `./gradlew :meeple-api:test --tests "com.org.meeple.api.admin.AdminRecommendedTeamBatchE2ETest" -q`
...
**Files:**
- Create: `meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/match/query/dto/CandidateTeam.kt`
- Create: `meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/match/command/domain/TeamPool.kt`
- Test: `meeple-api/src/test/kotlin/com/org/meeple/scheduler/match/TeamPoolTest.kt`
...
`meeple-api/src/test/kotlin/com/org/meeple/scheduler/match/TeamPoolTest.kt`:
...
Run: `./gradlew :meeple-api:test --tests "com.org.meeple.scheduler.match.TeamPoolTest" -q`
Expected: 컴파일 실패 (`CandidateTeam`/`TeamPool` 미존재)

- [ ] **Step 3: `CandidateTeam` 작성**

`meeple-scheduler/.../query/dto/CandidateTeam.kt`:
...
`meeple-scheduler/.../command/domain/TeamPool.kt`:
...
**Files:**
- Create: `meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/match/query/dao/GetRecommendedTeamRecordDao.kt`
- Create: `meeple-infra/src/main/kotlin/com/org/meeple/infra/match/query/GetRecommendedTeamRecordDaoImpl.kt`
...
`meeple-scheduler/.../query/dao/GetRecommendedTeamRecordDao.kt`:
...
`meeple-infra/.../match/query/GetRecommendedTeamRecordDaoImpl.kt`:
...
**Files:**
- Modify: `meeple-scheduler/.../query/dto/RecommendableSoloUser.kt`
- Modify: `meeple-scheduler/.../query/dao/GetRecommendableSoloUserDao.kt`
- Modify: `meeple-scheduler/.../query/dao/GetCandidateTeamDao.kt`
- Modify: `meeple-scheduler/.../command/application/RecommendedTeamBatchService.kt`
- Modify: `meeple-scheduler/.../command/application/port/in/RunRecommendedTeamBatchUseCase.kt` (KDoc)
- Modify: `meeple-infra/.../match/query/GetRecommendableSoloUserDaoImpl.kt`
- Modify: `meeple-infra/.../match/query/GetCandidateTeamDaoImpl.kt`
- Test: `meeple-api/src/test/kotlin/com/org/meeple/api/scheduler/RunRecommendedTeamBatchIntegrationTest.kt` (신규)
- Test: `meeple-api/src/test/kotlin/com/org/meeple/api/admin/AdminRecommendedTeamBatchE2ETest.kt` (갱신)
...
`meeple-api/src/test/kotlin/com/org/meeple/api/scheduler/RunRecommendedTeamBatchIntegrationTest.kt`:
...
Run: `./gradlew :meeple-api:test --tests "com.org.meeple.api.scheduler.RunRecommendedTeamBatchIntegrationTest" -q`
...
`meeple-scheduler/.../query/dto/RecommendableSoloUser.kt` 전체 교체:
...
`meeple-scheduler/.../query/dao/GetRecommendableSoloUserDao.kt` 전체 교체:
...
`meeple-scheduler/.../query/dao/GetCandidateTeamDao.kt` 전체 교체:
...
`meeple-scheduler/.../command/application/RecommendedTeamBatchService.kt` 전체 교체:
...
`meeple-scheduler/.../command/application/port/in/RunRecommendedTeamBatchUseCase.kt`의 KDoc을 새 동작에 맞춘다(시그니처는 동일):
...
`meeple-infra/.../match/query/GetRecommendableSoloUserDaoImpl.kt` 전체 교체 (regionId 투영 + 전체 반환):
...
`meeple-infra/.../match/query/GetCandidateTeamDaoImpl.kt` 전체 교체 (ACTIVE 팀 전체 → CandidateTeam):
...
`meeple-api/.../api/admin/AdminRecommendedTeamBatchE2ETest.kt`를 새 동작(근접 권역 + region 적재 필요)에 맞춰 교체한다:
