# meeple-admin 모듈 분리 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 어드민 전용 core-레이어 코드(대시보드·신고 조회 UseCase/Service/Port/View)를 운영 `meeple-core`에서 떼어내 새 자립 모듈 `meeple-admin`으로 옮긴다.

**Architecture:** `meeple-admin`은 `meeple-core`에 의존하지 않고 `meeple-common`만 쓰는 자립 모듈이다(`meeple-chatting`·`meeple-scheduler`와 동일 컨벤션). core에 결합돼 있던 `TimeGenerator`·`BusinessException`/`ErrorCode`는 admin 자체 포트/예외로 대체하고, 예외→HTTP 변환 핸들러만 `meeple-api`에 둔다. 어드민 컨트롤러(api)·어댑터(infra)는 기존 위치에 그대로 두고 import만 `admin.*`로 재지정한다.

**Tech Stack:** Kotlin 2.2.21 / JVM 21, Spring Boot 4.0.6 (Gradle 멀티모듈 + `meeple.kotlin-conventions`), QueryDSL(OpenFeign 포크), Kotest 5.9.1 + RestAssured E2E(Testcontainers).

## Global Constraints

- 언어/응답: 모든 커밋 메시지·주석은 한국어. 커밋 형식 `<type>(<domain>): <설명>` (본 작업 도메인은 `admin`).
- **`meeple-admin`은 `meeple-core`를 의존하지 않는다.** admin 소스에 `com.org.meeple.core.*` import 0건, build.gradle에 `project(":meeple-core")` 없음.
- **운영 core 파일 무변경**: `meeple-core`에서 삭제하는 것은 이동 대상 admin 파일뿐. 그 외 core 파일은 한 줄도 바꾸지 않는다.
...
- 검증 명령: 모듈 컴파일은 `./gradlew :meeple-admin:compileKotlin`, 전체는 `./gradlew compileKotlin compileTestKotlin`, 어드민 E2E는 아래 Task 8의 명령.

---

## File Structure

**새 모듈 `meeple-admin`** (base package `com.org.meeple.admin`):
- `meeple-admin/build.gradle.kts` — common + spring-context/tx/web 의존
...
- `settings.gradle.kts` — `include("meeple-admin")`
- `meeple-api/build.gradle.kts`, `meeple-infra/build.gradle.kts` — `implementation(project(":meeple-admin"))`
- `meeple-api/.../api/admin/AdminDashboardController.kt`, `AdminReportController.kt`
- `meeple-api/.../api/admin/response/AdminDashboardResponse.kt`, `AdminReportDetailResponse.kt`, `AdminReportPageResponse.kt`, `AdminReportSummaryResponse.kt`
- `meeple-api/.../api/admin/AdminExceptionHandler.kt` — **신규**
- `meeple-infra/.../infra/admin/query/GetAdminDashboardDaoImpl.kt`, `infra/report/query/GetAdminReportDaoImpl.kt`

**삭제(core에서 제거):** 위 이동 대상 core 파일 8종 + 비게 된 `core/admin`, `core/report/query` 디렉터리.

---

### Task 1: meeple-admin Gradle 모듈 뼈대 생성

새 모듈을 등록하고 빈 컴파일이 되도록 만든다. (아직 소스 없음)

**Files:**
- Modify: `settings.gradle.kts`
- Create: `meeple-admin/build.gradle.kts`

**Interfaces:**
- Consumes: 없음
- Produces: Gradle 프로젝트 `:meeple-admin` (common + spring-context/tx/web on compile classpath, Boot BOM 4.0.6 관리)

- [ ] **Step 1: `settings.gradle.kts`에 모듈 등록**

`include("meeple-scheduler")` 아래 줄에 추가:
...
- [ ] **Step 2: `meeple-admin/build.gradle.kts` 생성**
...
Run: `./gradlew :meeple-admin:compileKotlin`
Expected: BUILD SUCCESSFUL (소스가 없어도 성공)

- [ ] **Step 4: 커밋**

```bash
git add settings.gradle.kts meeple-admin/build.gradle.kts
git commit -m "build(admin): meeple-admin 자립 모듈 뼈대 추가"
```
...
**Files:**
- Create: `meeple-admin/src/main/kotlin/com/org/meeple/admin/common/time/TimeGenerator.kt`
- Create: `meeple-admin/src/main/kotlin/com/org/meeple/admin/common/time/SystemAdminTimeGenerator.kt`
- Create: `meeple-admin/src/main/kotlin/com/org/meeple/admin/common/error/AdminErrorCode.kt`
- Create: `meeple-admin/src/main/kotlin/com/org/meeple/admin/common/error/AdminException.kt`

**Interfaces:**
- Consumes: 없음
- Produces:
  - `com.org.meeple.admin.common.time.TimeGenerator` — `fun now(): LocalDateTime`, `fun today(): LocalDate`
  - `com.org.meeple.admin.common.time.SystemAdminTimeGenerator` — `@Component`, 위 인터페이스 구현
  - `com.org.meeple.admin.common.error.AdminErrorCode` — enum, 프로퍼티 `code: String`, `message: String`, `status: HttpStatus`; 항목 `REPORT_NOT_FOUND`
  - `com.org.meeple.admin.common.error.AdminException(errorCode: AdminErrorCode, message: String)` — `RuntimeException`
...
```bash
git add meeple-admin/src/main/kotlin/com/org/meeple/admin/common
...
**Files:**
- Create: `meeple-admin/.../admin/dashboard/query/dto/AdminDashboardView.kt`
- Create: `meeple-admin/.../admin/dashboard/query/dao/GetAdminDashboardDao.kt`
- Create: `meeple-admin/.../admin/dashboard/query/service/port/in/GetAdminDashboardUseCase.kt`
- Create: `meeple-admin/.../admin/dashboard/query/service/GetAdminDashboardService.kt`
- Delete: `meeple-core/.../core/admin/query/dto/AdminDashboardView.kt`, `dao/GetAdminDashboardDao.kt`, `service/port/in/GetAdminDashboardUseCase.kt`, `service/GetAdminDashboardService.kt` (그리고 빈 `core/admin` 디렉터리)

**Interfaces:**
- Consumes: `com.org.meeple.admin.common.time.TimeGenerator` (Task 2)
- Produces:
  - `com.org.meeple.admin.dashboard.query.dto.AdminDashboardView` — 7개 Long 프로퍼티 data class
  - `com.org.meeple.admin.dashboard.query.dao.GetAdminDashboardDao` — `fun load(todayStart: LocalDateTime): AdminDashboardView`
  - `com.org.meeple.admin.dashboard.query.service.port.in.GetAdminDashboardUseCase` — `fun get(): AdminDashboardView`
...
```bash
git rm meeple-core/src/main/kotlin/com/org/meeple/core/admin/query/dto/AdminDashboardView.kt \
       meeple-core/src/main/kotlin/com/org/meeple/core/admin/query/dao/GetAdminDashboardDao.kt \
       "meeple-core/src/main/kotlin/com/org/meeple/core/admin/query/service/port/in/GetAdminDashboardUseCase.kt" \
       meeple-core/src/main/kotlin/com/org/meeple/core/admin/query/service/GetAdminDashboardService.kt
```

빈 디렉터리 정리(`git rm`이 파일만 지우므로 남은 빈 폴더 제거):

```bash
find meeple-core/src/main/kotlin/com/org/meeple/core/admin -type d -empty -delete
```
...
```bash
git add meeple-admin meeple-core
git commit -m "refactor(admin): 대시보드 조회 코드를 meeple-admin으로 이동"
```
...
**Files:**
- Create: `meeple-admin/.../admin/report/query/dto/AdminReportDetailView.kt`, `AdminReportSummaryView.kt`, `AdminReportSummaryViews.kt`, `AdminReportPage.kt`
- Delete: 같은 4파일의 core 원본 (`meeple-core/.../core/report/query/dto/*`)

**Interfaces:**
- Consumes: `com.org.meeple.common.report.ReportStatus`, `com.org.meeple.common.report.ReportType` (common, 변경 없음)
- Produces:
  - `com.org.meeple.admin.report.query.dto.AdminReportDetailView` — data class(12필드)
  - `com.org.meeple.admin.report.query.dto.AdminReportSummaryView` — data class(10필드)
  - `com.org.meeple.admin.report.query.dto.AdminReportSummaryViews` — `val values: List<AdminReportSummaryView>` + `empty()`
  - `com.org.meeple.admin.report.query.dto.AdminReportPage` — `reports/page/size/totalElements` + 파생 `totalPages`/`hasNext` + `empty(page,size)`
...
```bash
git rm meeple-core/src/main/kotlin/com/org/meeple/core/report/query/dto/AdminReportDetailView.kt \
       meeple-core/src/main/kotlin/com/org/meeple/core/report/query/dto/AdminReportSummaryView.kt \
       meeple-core/src/main/kotlin/com/org/meeple/core/report/query/dto/AdminReportSummaryViews.kt \
       meeple-core/src/main/kotlin/com/org/meeple/core/report/query/dto/AdminReportPage.kt
```
...
```bash
git add meeple-admin meeple-core
git commit -m "refactor(admin): 신고 조회 read model을 meeple-admin으로 이동"
```
...
**Files:**
- Create: `meeple-admin/.../admin/report/query/dao/GetAdminReportDao.kt`
- Create: `meeple-admin/.../admin/report/query/service/port/in/GetAdminReportsUseCase.kt`
- Create: `meeple-admin/.../admin/report/query/service/GetAdminReportsService.kt`
...
- Produces:
  - `com.org.meeple.admin.report.query.dao.GetAdminReportDao` — `fun findPage(offset: Long, limit: Int): AdminReportSummaryViews`, `fun count(): Long`, `fun findDetailById(id: Long): AdminReportDetailView?`
  - `com.org.meeple.admin.report.query.service.port.in.GetAdminReportsUseCase` — `fun getReports(page: Int, size: Int): AdminReportPage`, `fun getReport(id: Long): AdminReportDetailView`
...
```bash
git rm meeple-core/src/main/kotlin/com/org/meeple/core/report/query/dao/GetAdminReportDao.kt \
       "meeple-core/src/main/kotlin/com/org/meeple/core/report/query/service/port/in/GetAdminReportsUseCase.kt" \
       meeple-core/src/main/kotlin/com/org/meeple/core/report/query/service/GetAdminReportsService.kt
find meeple-core/src/main/kotlin/com/org/meeple/core/report/query -type d -empty -delete
```
...
```bash
git add meeple-admin meeple-core
git commit -m "refactor(admin): 신고 조회 UseCase/Service를 meeple-admin으로 이동"
```

---

### Task 6: meeple-infra를 admin 포트에 재배선

infra가 `meeple-admin`을 의존하게 하고, 두 DAO 구현체의 import를 `core.*` → `admin.*`로 바꾼다. (엔티티 참조는 그대로)

**Files:**
- Modify: `meeple-infra/build.gradle.kts`
- Modify: `meeple-infra/.../infra/admin/query/GetAdminDashboardDaoImpl.kt` (import 3줄)
- Modify: `meeple-infra/.../infra/report/query/GetAdminReportDaoImpl.kt` (import 4줄)

**Interfaces:**
- Consumes: Task 3·4·5의 admin dao/dto 타입
- Produces: `GetAdminDashboardDao`/`GetAdminReportDao`(admin)의 `@Component` 구현 — 시그니처 불변

- [ ] **Step 1: `meeple-infra/build.gradle.kts`에 admin 의존 추가**

`implementation(project(":meeple-chatting"))` 아래에 추가:
...
```bash
git add meeple-infra
git commit -m "refactor(admin): infra 어드민 어댑터를 meeple-admin 포트에 재배선"
```

---

### Task 7: meeple-api를 admin UseCase에 재배선 + 예외 핸들러 추가

api가 `meeple-admin`을 의존하게 하고, 어드민 컨트롤러·응답 DTO의 import를 `admin.*`로 바꾼다. `AdminException`을 HTTP 응답으로 변환하는 핸들러를 추가한다.

**Files:**
- Modify: `meeple-api/build.gradle.kts`
- Modify: `meeple-api/.../api/admin/AdminDashboardController.kt` (import 1줄)
- Modify: `meeple-api/.../api/admin/AdminReportController.kt` (import 1줄)
- Modify: `meeple-api/.../api/admin/response/AdminDashboardResponse.kt` (import 1줄)
- Modify: `meeple-api/.../api/admin/response/AdminReportDetailResponse.kt` (import 1줄)
- Modify: `meeple-api/.../api/admin/response/AdminReportPageResponse.kt` (import 1줄)
- Modify: `meeple-api/.../api/admin/response/AdminReportSummaryResponse.kt` (import 1줄)
- Create: `meeple-api/.../api/admin/AdminExceptionHandler.kt`

**Interfaces:**
- Consumes: Task 2의 `AdminException`/`AdminErrorCode`; Task 3·5의 UseCase; Task 3·4의 View/Page; core의 `ApiResponse`/`ErrorResponse`(기존)
- Produces: 없음(경계)

- [ ] **Step 1: `meeple-api/build.gradle.kts`에 admin 의존 추가**

`implementation(project(":meeple-scheduler"))` 아래에 추가:
...
(두 파일 모두 `import com.org.meeple.core.common.response.ApiResponse`는 **그대로 둔다** — api는 core 의존)
...
```bash
git add meeple-api
git commit -m "refactor(admin): api 어드민 컨트롤러를 meeple-admin에 재배선하고 예외 핸들러 추가"
```
...
Expected: BUILD SUCCESSFUL — 남아 있는 `com.org.meeple.core.admin`/`core.report.query` 참조가 없어야 한다.

- [ ] **Step 2: admin이 core를 참조하지 않는지 검증**

Run:
```bash
grep -rn "com.org.meeple.core" meeple-admin/src 2>/dev/null; echo "exit=$?"
```
Expected: 매치 없음(출력 없고 `exit=1`).

Run:
```bash
grep -n "meeple-core" meeple-admin/build.gradle.kts; echo "exit=$?"
```
...
Run:
```bash
./gradlew :meeple-api:test --tests "com.org.meeple.api.admin.*" --tests "com.org.meeple.api.auth.AdminAccessE2ETest"
```
...
```bash
git add -A
git commit -m "chore(admin): meeple-admin 분리 후 빌드·E2E 검증"
```
