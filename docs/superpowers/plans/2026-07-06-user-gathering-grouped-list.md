...
**Architecture:** 헥사고날 + CQRS의 조회 전용(query-only) 경로. 유저는 모임을 생성하지 않으므로 command 측 없이 `oneulsogae-core`에 유저용 gathering query 도메인을 신설한다(기존 `oneulsogae-admin`의 `AdminGathering`과 별개, 같은 `gatherings` 테이블을 읽는 독립 read model). 컨트롤러는 `oneulsogae-api`, QueryDSL DAO 구현은 `oneulsogae-infra`에 두고 이미지 presign은 기존 S3 어댑터를 재사용한다.
...
- 응답·주석·문서는 한국어로 작성한다.
- `oneulsogae-backend`만 수정한다. 프론트엔드는 건드리지 않는다(아래 "프론트엔드 안내" 참고).
...
- 패키지 베이스: core = `com.org.oneulsogae.core.<domain>`, infra = `com.org.oneulsogae.infra.<domain>`, api = `com.org.oneulsogae.api.<domain>`, 공용 enum = `com.org.oneulsogae.common.<domain>`.
...
**신규 (oneulsogae-core — `com.org.oneulsogae.core.gathering.query`)**
...
**신규 (oneulsogae-infra — `com.org.oneulsogae.infra.gathering.query`)**
...
**신규 (oneulsogae-api — `com.org.oneulsogae.api.gathering`)**
- `GatheringController.kt` — `GET /gatherings/v1`.
- `response/GatheringGroupListResponse.kt` — 응답 DTO(groups/Group/Item + `from`).

**수정 (oneulsogae-api 테스트)**
- `common/config/TestFileStorageConfig.kt` — core `GatheringImageUrlPort` 페이크 빈 추가.

**신규 (테스트)**
- `oneulsogae-api/src/test/.../domain/gathering/GatheringViewsTest.kt` — `groupByType` 유닛 테스트(Kotest).
- `oneulsogae-api/src/test/.../api/gathering/GatheringQueryE2ETest.kt` — 엔드포인트 E2E.
...
**Files:**
- Create: `oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/gathering/query/dto/GatheringView.kt`
- Create: `oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/gathering/query/dto/GatheringTypeGroup.kt`
- Create: `oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/gathering/query/dto/GroupedGatherings.kt`
- Create: `oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/gathering/query/dto/GatheringViews.kt`
- Test: `oneulsogae-api/src/test/kotlin/com/org/oneulsogae/domain/gathering/GatheringViewsTest.kt`
...
Run: `./gradlew :oneulsogae-api:test --tests "com.org.oneulsogae.domain.gathering.GatheringViewsTest"`
Expected: PASS (3 tests). (DTO를 먼저 만들었으므로 컴파일·통과한다. 만약 Step 1~2를 건너뛰면 컴파일 실패로 FAIL — 그 경우 DTO부터 만든다.)

- [ ] **Step 5: 커밋**

```bash
git add oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/gathering \
        oneulsogae-api/src/test/kotlin/com/org/oneulsogae/domain/gathering/GatheringViewsTest.kt
...
**Files:**
- Create: `oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/gathering/query/dao/GetGatheringDao.kt`
- Create: `oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/gathering/query/service/port/in/GetGatheringsUseCase.kt`
- Create: `oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/gathering/query/service/port/out/GatheringImageUrlPort.kt`
- Create: `oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/gathering/query/service/GetGatheringsService.kt`
...
```kotlin
package com.org.oneulsogae.core.gathering.query.service.port.out
...
```bash
git add oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/gathering/query
...
**Files:**
- Create: `oneulsogae-infra/src/main/kotlin/com/org/oneulsogae/infra/gathering/query/GetGatheringDaoImpl.kt`
- Modify: `oneulsogae-infra/src/main/kotlin/com/org/oneulsogae/infra/gathering/query/S3GatheringImageUrlAdapter.kt`

**Interfaces:**
- Consumes (Task 1·2): `GatheringView`, `GatheringViews`, `GetGatheringDao`, core `GatheringImageUrlPort`.
- Consumes (기존): `QGatheringEntity.gatheringEntity`(생성된 Q타입), `JPAQueryFactory`, `com.org.oneulsogae.common.gathering.GatheringStatus`.
...
```bash
git add oneulsogae-infra/src/main/kotlin/com/org/oneulsogae/infra/gathering/query/GetGatheringDaoImpl.kt \
        oneulsogae-infra/src/main/kotlin/com/org/oneulsogae/infra/gathering/query/S3GatheringImageUrlAdapter.kt
...
**Files:**
- Create: `oneulsogae-api/src/main/kotlin/com/org/oneulsogae/api/gathering/response/GatheringGroupListResponse.kt`
- Create: `oneulsogae-api/src/main/kotlin/com/org/oneulsogae/api/gathering/GatheringController.kt`
...
```kotlin
package com.org.oneulsogae.api.gathering.response

import com.org.oneulsogae.common.gathering.GatheringType
import com.org.oneulsogae.core.gathering.query.dto.GatheringTypeGroup
import com.org.oneulsogae.core.gathering.query.dto.GatheringView
import com.org.oneulsogae.core.gathering.query.dto.GroupedGatherings
import java.time.LocalDateTime
...
import com.org.oneulsogae.api.gathering.response.GatheringGroupListResponse
...
```bash
git add oneulsogae-api/src/main/kotlin/com/org/oneulsogae/api/gathering
...
**Files:**
- Modify: `oneulsogae-api/src/test/kotlin/com/org/oneulsogae/common/config/TestFileStorageConfig.kt`
- Test: `oneulsogae-api/src/test/kotlin/com/org/oneulsogae/api/gathering/GatheringQueryE2ETest.kt`
...
Run: `./gradlew :oneulsogae-api:test --tests "com.org.oneulsogae.api.gathering.GatheringQueryE2ETest"`
...
git add oneulsogae-api/src/test/kotlin/com/org/oneulsogae/common/config/TestFileStorageConfig.kt \
        oneulsogae-api/src/test/kotlin/com/org/oneulsogae/api/gathering/GatheringQueryE2ETest.kt
...
Run: `./gradlew :oneulsogae-api:test --tests "com.org.oneulsogae.domain.gathering.GatheringViewsTest" --tests "com.org.oneulsogae.api.gathering.GatheringQueryE2ETest"`
Expected: PASS (8 tests).

- [ ] **Step 2: 기존 어드민 gathering 테스트가 깨지지 않았는지 확인한다** (S3 어댑터·TestFileStorageConfig를 건드렸으므로 회귀 확인)

Run: `./gradlew :oneulsogae-api:test --tests "com.org.oneulsogae.api.admin.AdminGathering*"`
