...
**Architecture:** 배치와 실시간 추가소개가 동일한 매칭 알고리즘을 쓰도록 `MatchScorer`/`MatchSelector`를 새 순수 모듈 `oneulsogae-matching`으로 분리한다. 조회는 CQRS query, 추가소개는 command로 구현하고, 코인 차감·매칭 생성·기존 interest 흐름을 재사용한다.
...
**신규 모듈 `oneulsogae-matching`** (`com.org.oneulsogae.matching`)
...
## Task 1: `oneulsogae-matching` 모듈 생성 + 알고리즘 이동

**Files:**
- Create: `oneulsogae-matching/build.gradle.kts`
- Modify: `settings.gradle`
- Create: `oneulsogae-matching/src/main/kotlin/com/org/oneulsogae/matching/MatchScoringProfile.kt`
- Create: `oneulsogae-matching/src/main/kotlin/com/org/oneulsogae/matching/MatchScorer.kt`
- Delete: `oneulsogae-scheduler/.../solomatch/command/domain/MatchScorer.kt`, `oneulsogae-scheduler/.../solomatch/query/dto/MatchScoringProfile.kt`

**Interfaces:**
- Produces: `com.org.oneulsogae.matching.MatchScoringProfile`(기존 필드 동일), `com.org.oneulsogae.matching.MatchScorer`(모든 함수 유지 + `fun <T> orderByScore(scored: List<Pair<T, Double>>, random: Random): List<T>`).

- [ ] **Step 1: settings.gradle에 모듈 등록**

`settings.gradle`의 include 목록에 추가:
```groovy
include("oneulsogae-matching")
```

- [ ] **Step 2: build.gradle.kts 작성**

`oneulsogae-matching/build.gradle.kts`:
```kotlin
plugins {
...
`oneulsogae-matching/.../matching/MatchScoringProfile.kt` — 기존 scheduler `query/dto/MatchScoringProfile.kt` 내용을 그대로 옮기되 package를 `com.org.oneulsogae.matching`으로 변경. (필드/주석 동일)

- [ ] **Step 4: MatchScorer 이동 + orderByScore 제네릭화**

`oneulsogae-matching/.../matching/MatchScorer.kt`:
```kotlin
package com.org.oneulsogae.matching
...
scheduler의 `command/domain/MatchScorer.kt`, `query/dto/MatchScoringProfile.kt` 삭제. `MatchScoringProfile`/`MatchScorer`를 참조하는 scheduler 파일들(`SoloMatchBatchService`, `GetMatchScoringProfileDao`, `query/dto` 소비처)의 import를 `com.org.oneulsogae.matching.*`로 교체. (Task 1은 Task 2에서 함께 컴파일 검증)

- [ ] **Step 6: scheduler build.gradle에 의존 추가**

`oneulsogae-scheduler/build.gradle`:
```groovy
	implementation(project(":oneulsogae-matching"))
```

- [ ] **Step 7: infra build.gradle에 의존 추가**

`oneulsogae-infra/build.gradle` dependencies에 추가(무브된 `MatchScoringProfile`을 infra의 `GetMatchScoringProfileDaoImpl`이 참조):
```groovy
	implementation(project(":oneulsogae-matching"))
```
...
**Files:**
- Create: `oneulsogae-matching/src/main/kotlin/com/org/oneulsogae/matching/ScoringCandidate.kt`
- Create: `oneulsogae-matching/src/main/kotlin/com/org/oneulsogae/matching/MatchSelector.kt`
- Modify: `oneulsogae-scheduler/.../solomatch/query/dto/MatchableUser.kt`
- Modify: `oneulsogae-scheduler/.../solomatch/command/application/SoloMatchBatchService.kt`
- Test: `oneulsogae-api/src/test/kotlin/com/org/oneulsogae/matching/MatchSelectorTest.kt`
...
`oneulsogae-matching/.../matching/ScoringCandidate.kt`:
```kotlin
package com.org.oneulsogae.matching
...
`oneulsogae-matching/.../matching/MatchSelector.kt`:
```kotlin
package com.org.oneulsogae.matching
...
`oneulsogae-scheduler/.../solomatch/query/dto/MatchableUser.kt`:
```kotlin
package com.org.oneulsogae.scheduler.solomatch.query.dto

import com.org.oneulsogae.common.user.Gender
import com.org.oneulsogae.matching.ScoringCandidate
import java.time.LocalDateTime
...
`SoloMatchBatchService.findBestFreshPartner`를 다음으로 교체(동작 동일). import에서 `MatchScorer` 제거, `com.org.oneulsogae.matching.MatchScoringProfile`, `com.org.oneulsogae.matching.MatchSelector` 추가:
```kotlin
...
Run: `./gradlew :oneulsogae-matching:compileKotlin :oneulsogae-scheduler:compileKotlin :oneulsogae-infra:compileKotlin`
Expected: BUILD SUCCESSFUL

- [ ] **Step 6: MatchSelector 유닛 테스트 작성**

`oneulsogae-api/src/test/kotlin/com/org/oneulsogae/matching/MatchSelectorTest.kt`:
```kotlin
package com.org.oneulsogae.matching
...
Run: `./gradlew :oneulsogae-api:test --tests "com.org.oneulsogae.matching.MatchSelectorTest"`
Expected: PASS

- [ ] **Step 8: 배치 회귀 테스트 실행**

Run: `./gradlew :oneulsogae-api:test --tests "*SoloMatchBatch*"`
Expected: PASS (배치 동작 불변)

- [ ] **Step 9: 커밋**

```bash
git add oneulsogae-matching settings.gradle oneulsogae-scheduler oneulsogae-infra/build.gradle oneulsogae-api/src/test/kotlin/com/org/oneulsogae/matching
git commit -m "refactor(match): 매칭 스코어링·선택 알고리즘을 oneulsogae-matching 모듈로 분리
...
**Files:**
- Modify: `oneulsogae-common/.../coin/CoinUsageType.kt`
- Modify: `oneulsogae-common/.../match/SoloMatchType.kt`
- Modify: `oneulsogae-core/.../solomatch/MatchErrorCode.kt`
- Modify: `oneulsogae-core/.../common/lock/LockKeyConstraints.kt`
...
Run: `./gradlew :oneulsogae-common:compileKotlin :oneulsogae-core:compileKotlin`
Expected: BUILD SUCCESSFUL

- [ ] **Step 6: 커밋**

```bash
git add oneulsogae-common oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/solomatch/MatchErrorCode.kt oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/common/lock/LockKeyConstraints.kt
...
**Files:**
- Create: `oneulsogae-core/.../solomatch/query/dao/dto/ExtraIntroCandidate.kt`
- Create: `oneulsogae-core/.../solomatch/query/dao/dto/ExtraIntroCandidates.kt`
- Create: `oneulsogae-core/.../solomatch/query/dao/dto/ExtraIntroScoringRow.kt`
- Create: `oneulsogae-core/.../solomatch/query/dao/GetExtraIntroCandidateDao.kt`
- Create: `oneulsogae-core/.../common/region/GetRegionProximityPort.kt`
- Create: `oneulsogae-core/.../solomatch/query/service/port/in/GetExtraIntroCandidatesUseCase.kt`
- Create: `oneulsogae-core/.../solomatch/query/service/GetExtraIntroCandidatesService.kt`
...
```kotlin
package com.org.oneulsogae.core.solomatch.query.dao.dto

import com.org.oneulsogae.matching.MatchScoringProfile
import com.org.oneulsogae.matching.ScoringCandidate
import java.time.LocalDateTime
...
```kotlin
package com.org.oneulsogae.core.solomatch.query.dao

import com.org.oneulsogae.common.user.Gender
import com.org.oneulsogae.core.solomatch.query.dao.dto.ExtraIntroCandidate
import com.org.oneulsogae.core.solomatch.query.dao.dto.ExtraIntroScoringRow
import com.org.oneulsogae.matching.MatchScoringProfile
import java.time.LocalDate
import java.time.LocalDateTime
...
`oneulsogae-core/.../common/region/GetRegionProximityPort.kt`:
```kotlin
package com.org.oneulsogae.core.common.region
...
import com.org.oneulsogae.core.solomatch.query.dao.dto.ExtraIntroCandidates
...
import com.org.oneulsogae.matching.MatchScoringProfile
import com.org.oneulsogae.matching.MatchSelector
...
```bash
git add oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/solomatch/query oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/common/region
...
**Files:**
- Create: `oneulsogae-core/.../solomatch/command/application/port/in/IntroduceExtraMatchUseCase.kt`
- Create: `oneulsogae-core/.../solomatch/command/application/port/out/GetExtraIntroCandidatePort.kt` (+ `ExtraIntroCandidateRow`)
- Create: `oneulsogae-core/.../solomatch/command/application/IntroduceExtraMatchService.kt`
...
```bash
git add oneulsogae-core/src/main/kotlin/com/org/oneulsogae/core/solomatch
...
**Files:**
- Create: `oneulsogae-infra/.../solomatch/query/GetExtraIntroCandidateDaoImpl.kt`
- Create: `oneulsogae-infra/.../solomatch/command/adapter/ExtraIntroCandidateAdapter.kt`
- Create: `oneulsogae-infra/.../region/GetRegionProximityAdapter.kt`
...
`oneulsogae-infra/.../solomatch/query/GetExtraIntroCandidateDaoImpl.kt`. 핵심 규칙:
...
`oneulsogae-infra/.../solomatch/command/adapter/ExtraIntroCandidateAdapter.kt` — `GetExtraIntroCandidatePort` 구현.
...
`oneulsogae-infra/.../region/GetRegionProximityAdapter.kt`:
```kotlin
...
```bash
git add oneulsogae-infra/src/main/kotlin/com/org/oneulsogae/infra
...
**Files:**
- Create: `oneulsogae-api/.../match/response/ExtraIntroCandidatesResponse.kt`
- Create: `oneulsogae-api/.../match/response/ExtraIntroResponse.kt`
- Modify: `oneulsogae-api/.../match/SoloMatchController.kt` (엔드포인트 2개 추가)
...
```bash
git add oneulsogae-api/src/main/kotlin/com/org/oneulsogae/api/match
...
**Files:**
- Create: `oneulsogae-api/src/test/kotlin/com/org/oneulsogae/api/match/ExtraIntroCandidatesIntegrationTest.kt`
- Create: `oneulsogae-api/src/test/kotlin/com/org/oneulsogae/api/match/ExtraIntroIntegrationTest.kt`
...
Run: `./gradlew :oneulsogae-api:test --tests "*ExtraIntroCandidatesIntegrationTest"`
Expected: PASS
...
Run: `./gradlew :oneulsogae-api:test --tests "*ExtraIntroIntegrationTest"`
Expected: PASS
...
```bash
git add oneulsogae-api/src/test/kotlin/com/org/oneulsogae/api/match
