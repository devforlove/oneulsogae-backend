...
- **scheduler 모듈은 core·infra에 의존하지 않는다.** scheduler 코드는 `meeple-common`의 enum과 JDK만 쓴다. 나이 계산 등 core 유틸(`ageAt`)은 **infra 어댑터에서만** 사용한다.
...
- **스코어링 유닛 테스트는 `meeple-api/src/test`** 아래에 둔다(scheduler 도메인 유닛 테스트의 기존 관례; scheduler 모듈에는 test 의존성이 없다).
...
**신규 (scheduler)**
- `meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/query/dto/MatchScoringProfile.kt` — 스코어링용 read model(속성 + 이상형). 순수 데이터.
- `meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/command/domain/MatchScorer.kt` — 순수 점수 계산기(이상형 부합·거리·최근·종합·정렬). 프레임워크 무의존.
- `meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/query/dao/GetMatchScoringProfileDao.kt` — 조회 포트.

**신규 (infra)**
- `meeple-infra/src/main/kotlin/com/org/meeple/infra/solomatch/query/GetMatchScoringProfileDaoImpl.kt` — `user_details`+`user_ideal_types` 조인 투영.

**수정 (scheduler)**
- `meeple-scheduler/.../solomatch/command/domain/MatchPool.kt` — `availableCandidates(gender)` 추가.
- `meeple-scheduler/.../solomatch/command/application/SoloMatchBatchService.kt` — 스코어링 기반 후보 선택으로 교체.

**신규 테스트 (meeple-api)**
- `meeple-api/src/test/kotlin/com/org/meeple/scheduler/solomatch/MatchScorerTest.kt`
- `meeple-api/src/test/kotlin/com/org/meeple/scheduler/solomatch/SoloMatchBatchServiceTest.kt`
- `meeple-api/src/test/kotlin/com/org/meeple/api/scheduler/GetMatchScoringProfileDaoIntegrationTest.kt`

**수정 테스트 (meeple-api)**
- `meeple-api/src/test/kotlin/com/org/meeple/scheduler/solomatch/MatchPoolTest.kt` — `availableCandidates` 케이스 추가.
- `meeple-api/src/test/kotlin/com/org/meeple/api/scheduler/RunSoloMatchBatchIntegrationTest.kt` — 이상형 우선순위 시나리오 추가.
...
**Files:**
- Create: `meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/query/dto/MatchScoringProfile.kt`
- Create: `meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/command/domain/MatchScorer.kt`
- Test: `meeple-api/src/test/kotlin/com/org/meeple/scheduler/solomatch/MatchScorerTest.kt`
...
Run: `./gradlew :meeple-api:test --tests "com.org.meeple.scheduler.solomatch.MatchScorerTest"`
Expected: 컴파일 실패(`MatchScoringProfile`·`MatchScorer` 미존재).

- [ ] **Step 3: read model 생성**

`meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/query/dto/MatchScoringProfile.kt`:
...
`meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/command/domain/MatchScorer.kt`:
...
```bash
git add meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/query/dto/MatchScoringProfile.kt \
        meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/command/domain/MatchScorer.kt \
        meeple-api/src/test/kotlin/com/org/meeple/scheduler/solomatch/MatchScorerTest.kt
...
**Files:**
- Modify: `meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/command/domain/MatchScorer.kt`
- Test: `meeple-api/src/test/kotlin/com/org/meeple/scheduler/solomatch/MatchScorerTest.kt` (append)
...
```bash
git add meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/command/domain/MatchScorer.kt \
        meeple-api/src/test/kotlin/com/org/meeple/scheduler/solomatch/MatchScorerTest.kt
...
**Files:**
- Modify: `meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/command/domain/MatchPool.kt`
- Test: `meeple-api/src/test/kotlin/com/org/meeple/scheduler/solomatch/MatchPoolTest.kt` (append)
...
```kotlin
    describe("availableCandidates") {
        ...
        fun u(userId: Long, gender: com.org.meeple.common.user.Gender, regionId: Long): com.org.meeple.scheduler.solomatch.query.dto.MatchableUser =
            com.org.meeple.scheduler.solomatch.query.dto.MatchableUser(userId, gender, regionId, now)

        it("지역과 무관하게 해당 성별의 가용 후보를 모두 돌려준다") {
            val pool: com.org.meeple.scheduler.solomatch.command.domain.MatchPool =
                com.org.meeple.scheduler.solomatch.command.domain.MatchPool.of(
                    listOf(
                        u(1L, com.org.meeple.common.user.Gender.FEMALE, 10L),
                        u(2L, com.org.meeple.common.user.Gender.FEMALE, 20L),
                        u(3L, com.org.meeple.common.user.Gender.MALE, 10L),
                    ),
                )

            pool.availableCandidates(com.org.meeple.common.user.Gender.FEMALE)
                .map { it.userId }.sorted() shouldBe listOf(1L, 2L)
        }

        it("remove된 후보는 제외된다") {
            val female1: com.org.meeple.scheduler.solomatch.query.dto.MatchableUser = u(1L, com.org.meeple.common.user.Gender.FEMALE, 10L)
            val pool: com.org.meeple.scheduler.solomatch.command.domain.MatchPool =
                com.org.meeple.scheduler.solomatch.command.domain.MatchPool.of(
                    listOf(female1, u(2L, com.org.meeple.common.user.Gender.FEMALE, 20L)),
                )

            pool.remove(female1)

            pool.availableCandidates(com.org.meeple.common.user.Gender.FEMALE).map { it.userId } shouldBe listOf(2L)
        }
    }
```

> 참고: 기존 `MatchPoolTest.kt`가 이미 `import com.org.meeple.common.user.Gender` 등을 갖고 있으면 위 FQN을 그 import에 맞춰 축약해도 된다. FQN 그대로도 컴파일된다.

- [ ] **Step 2: 테스트 실패 확인**

Run: `./gradlew :meeple-api:test --tests "com.org.meeple.scheduler.solomatch.MatchPoolTest"`
...
```bash
git add meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/command/domain/MatchPool.kt \
        meeple-api/src/test/kotlin/com/org/meeple/scheduler/solomatch/MatchPoolTest.kt
...
**Files:**
- Create: `meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/query/dao/GetMatchScoringProfileDao.kt`
- Create: `meeple-infra/src/main/kotlin/com/org/meeple/infra/solomatch/query/GetMatchScoringProfileDaoImpl.kt`
- Test: `meeple-api/src/test/kotlin/com/org/meeple/api/scheduler/GetMatchScoringProfileDaoIntegrationTest.kt`

**Interfaces:**
- Consumes: `MatchScoringProfile`(Task 1), core `com.org.meeple.core.common.time.ageAt(today: LocalDate): Int`(infra에서만 사용), QueryDSL `QUserDetailEntity`·`QUserIdealTypeEntity`.
...
Run: `./gradlew :meeple-api:test --tests "com.org.meeple.api.scheduler.GetMatchScoringProfileDaoIntegrationTest"`
Expected: 컴파일 실패(`GetMatchScoringProfileDao` 미존재).

- [ ] **Step 3: 포트 생성**

`meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/query/dao/GetMatchScoringProfileDao.kt`:
...
`meeple-infra/src/main/kotlin/com/org/meeple/infra/solomatch/query/GetMatchScoringProfileDaoImpl.kt`:
...
> 실패 시 점검: `ageAt` import 경로(`com.org.meeple.core.common.time.ageAt`), `UserIdealTypeEntity` 생성자 파라미터명(`ageMin`/`ageMax`/`maritalStatus`), `Tuple` import(`com.querydsl.core.Tuple`).

- [ ] **Step 6: 커밋**

```bash
git add meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/query/dao/GetMatchScoringProfileDao.kt \
        meeple-infra/src/main/kotlin/com/org/meeple/infra/solomatch/query/GetMatchScoringProfileDaoImpl.kt \
        meeple-api/src/test/kotlin/com/org/meeple/api/scheduler/GetMatchScoringProfileDaoIntegrationTest.kt
...
**Files:**
- Modify: `meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/command/application/SoloMatchBatchService.kt`
- Test: `meeple-api/src/test/kotlin/com/org/meeple/scheduler/solomatch/SoloMatchBatchServiceTest.kt`
...
> 참고: `MatchedUserIds`의 실제 위치·생성자를 Task 시작 시 확인한다(`grep -rn "class MatchedUserIds" meeple-scheduler/src/main`). 위 import·생성 방식은 그 정의에 맞춘다. `GetMatchRecordDao`의 메서드 시그니처도 실제 인터페이스와 일치시킨다.

- [ ] **Step 2: 테스트 실패 확인**

Run: `./gradlew :meeple-api:test --tests "com.org.meeple.scheduler.solomatch.SoloMatchBatchServiceTest"`
...
```bash
git add meeple-scheduler/src/main/kotlin/com/org/meeple/scheduler/solomatch/command/application/SoloMatchBatchService.kt \
        meeple-api/src/test/kotlin/com/org/meeple/scheduler/solomatch/SoloMatchBatchServiceTest.kt
...
```bash
git add meeple-api/src/test/kotlin/com/org/meeple/api/scheduler/RunSoloMatchBatchIntegrationTest.kt
