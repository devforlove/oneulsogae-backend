# oneulsogae-admin 모듈 분리 설계

- 날짜: 2026-07-05
- 상태: 승인 대기
- 승인된 접근: **A안 — oneulsogae-admin 자립 모듈(core 미의존)**

## 배경 / 목적

어드민 전용으로 추가된 UseCase·Service·구현체가 운영(일반 사용자) 코드와 같은 모듈(`oneulsogae-core`)에 섞여 있다. 운영 core와 어드민 비즈니스 코드를 물리적으로 분리하기 위해 새 Gradle 모듈 `oneulsogae-admin`을 만든다.

사용자가 정한 원칙:

- `oneulsogae-api`, `oneulsogae-infra`는 운영 코드와 **동일하게 공유**한다(어드민 컨트롤러·어댑터는 그대로 이 모듈들에 둔다).
- `oneulsogae-core`는 **운영 전용**으로 유지한다.
- **어드민은 `oneulsogae-core`를 의존하지 않는다.** 어드민의 "core 역할"(도메인/유스케이스/서비스/포트)은 새 `oneulsogae-admin` 모듈이 맡는다. `oneulsogae-common`만 의존한다. (`oneulsogae-chatting`·`oneulsogae-scheduler`와 동일한 자립-모듈 컨벤션.)

## 이동 대상 (현재 → 이후)

### oneulsogae-core에서 oneulsogae-admin으로 이동 (repackage)

대시보드 (기존 `core.admin.query.*`) → `com.org.oneulsogae.admin.dashboard.query.*`
...
신고 어드민 조회 (기존 `core.report.query.*` 중 Admin 계열) → `com.org.oneulsogae.admin.report.query.*`
...
## oneulsogae-admin이 신설·소유할 core 대체 primitive
...
- `HttpStatus` 사용을 위해 oneulsogae-admin은 `spring-web`을 의존한다(core도 동일). 이는 spring 라이브러리 의존일 뿐 oneulsogae-core 의존이 아니다.
...
```
oneulsogae-admin ──> common, (spring-context, spring-tx, spring-web)   [어드민 core 역할. oneulsogae-core 미의존]
oneulsogae-api   ──> ..., oneulsogae-admin        [어드민 컨트롤러가 admin UseCase 주입. 신규 의존 추가]
oneulsogae-infra ──> ..., oneulsogae-admin        [어드민 DAO 구현이 admin out-port 구현. 신규 의존 추가]
```

- `settings.gradle.kts`에 `include("oneulsogae-admin")` 추가.
- `oneulsogae-admin/build.gradle.kts`: `implementation(project(":oneulsogae-common"))` + `spring-context` + `spring-tx` + `spring-web`. (scheduler build를 기준으로 tx·web 추가)
- `oneulsogae-api/build.gradle.kts`, `oneulsogae-infra/build.gradle.kts`에 `implementation(project(":oneulsogae-admin"))` 추가.
- 컴포넌트 스캔: `@SpringBootApplication`이 `com.org.oneulsogae`에 있어 `com.org.oneulsogae.admin.*`의 `@Service`/`@Component`가 자동 등록된다(별도 스캔 설정 불필요).

## oneulsogae-api / oneulsogae-infra의 import 재지정 (파일 이동 없음)
...
- `oneulsogae-common`의 `Role`(USER/ADMIN), `ReportStatus`/`ReportType` enum — 공유 유지.
...
- E2E(`oneulsogae-api/src/test`의 `AdminDashboardE2ETest`, `AdminReportDetailE2ETest`, `AdminReportListE2ETest`, `api/auth/AdminAccessE2ETest`): 실서버 HTTP를 두드리므로 **동작 불변**. core admin 타입을 직접 import하는 곳이 있으면 import만 `admin.*`로 재지정.
...
2. `oneulsogae-admin`이 `oneulsogae-core`를 의존하지 않음 — build.gradle에 core 없음 + admin 소스에 `com.org.oneulsogae.core.*` import 0건.
3. 어드민 E2E 전부 통과(대시보드 조회, 신고 목록/상세, 신고 단건 404, 어드민 접근 제어).
4. 운영 core 파일 무변경(diff에 `oneulsogae-core`의 비-admin 파일 변경 없음), `core/report/query`·`core/admin` 빈 패키지 정리됨.
