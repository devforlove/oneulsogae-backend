...
### 미인증 사용자도 매칭 풀에 남는다 (의도된 설계)

미인증 사용자도 매칭 풀에서 빠지지 않고 상대의 소개·미팅 리스트에 계속 노출된다. `MatchProfileSnapshot.companyName`은
선택 필드(`String?`)라 회사 미인증이어도 "매칭 가능"에는 영향이 없고, `CompleteOnboardingService.complete`는 온보딩
직후(`justOnboarded`) `syncMatchUser` → `recommendMatchUseCase.recommend` / `recommendTeamUseCase.recommend`를 회사
인증 여부와 무관하게 곧바로 돌린다. **의도된 설계다**: 관심을 받았다는 사실은 알 수 있게 하되, 그 관심을 확인하고
응답하려면 회사 인증을 하게 만들어 인증 전환을 유도하는 장치다.

다만 귀결은 정직하게 남긴다: 인증된 사용자가 미인증 상대에게 관심을 보내면 코인은 정상 차감되지만, 상대가 그 관심에
응답(수락 등)하는 경로는 명령 게이트에서 403을 받으므로 매칭이 성사되지 않는다. 배포 초기처럼 미인증 사용자 비율이
높은 구간에서는 이 "헛코인" 사례가 늘어날 수 있어 모니터링 대상이다.
