...
- `AdminErrorCode` (enum): core `ErrorCode` 계약(`code`, `message`, `status: HttpStatus`)을 그대로 본떠 admin 자체 정의. 현재 유일 항목 `REPORT_NOT_FOUND("ADMIN-REPORT-001", "신고를 찾을 수 없습니다.", HttpStatus.NOT_FOUND)`.
...
- 신규 admin primitive(`AdminErrorCode`/`AdminException`/`TimeGenerator`)는 로직이 거의 없으므로 별도 유닛 테스트를 강제하지 않는다. `AdminException` → 404 매핑은 기존 `AdminReportDetailE2ETest`(단건 조회 실패 케이스가 있다면)로 커버되는지 확인하고, 없으면 not-found E2E를 보강한다.
...
- `AdminException` 응답 포맷을 core `GlobalExceptionHandler`의 `BusinessException` 응답과 **동일하게** 맞추지 않으면 어드민 프론트가 파싱하는 에러 응답 스키마가 달라질 수 있다 → 구현 시 core 핸들러의 응답 본문 구조를 그대로 복제.
- (미결) `AdminReportDetailE2ETest`에 not-found(404) 케이스가 이미 있는지 확인 필요 — 없으면 보강 대상.
