...
### 1) 팝업 타입 (oneulsogae-common)

- `PopupType`에 `MEETING_FAILED_REFUND("미팅 매칭 실패 환불")` 추가. (솔로 `MATCH_FAILED_REFUND`와 구분)

### 2) 팀 지불자 추적 — `applicant_user_id` (oneulsogae-core 도메인 + oneulsogae-infra)
...
### 3) 매치별 만료 처리 (oneulsogae-core, match/command)
...
### 4) 환불 팝업 (oneulsogae-core, popup/command)
...
### 5) 배치 트리거 (oneulsogae-scheduler + oneulsogae-infra + oneulsogae-api)
...
- **oneulsogae-api**
  ...
  - `application.yml`: `oneulsogae.match.expire-batch.cron` 추가 — 운영 매시간(`0 0 * * * *`), `local` 프로파일은 매분.
