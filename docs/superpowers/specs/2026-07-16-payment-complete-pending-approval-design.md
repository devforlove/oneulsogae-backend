...
`GatheringMemberStatus`(oneulsogae-common)에 두 상태 추가:
...
`PaymentEntity`(oneulsogae-infra `payments/command/entity`): `user_id`, `gathering_id`,
...
`RegisterGatheringMemberService`(gathering command, oneulsogae-core 신설) 규칙 — 검증은 도메인
...
기존 admin 패턴(서비스 `oneulsogae-admin/gathering` + 컨트롤러 `oneulsogae-api/api/admin`)을 따른다.
...
- E2E(oneulsogae-api): 결제완료 정상 접수(멤버 PENDING + 결제 기록 + 여분 차감), 매진 거부,
