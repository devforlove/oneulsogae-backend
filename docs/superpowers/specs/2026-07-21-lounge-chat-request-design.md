...
`partnerProfileImageCode`·`partnerActivityArea`는 상대 프로필에서 조인해 온 표시용 값이며, 미설정이면 null이다. (신청 자체는 목록에서 빠지지 않는다)
