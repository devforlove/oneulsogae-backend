...
- **도메인 모델(`oneulsogae-core`의 `domain`) → 유닛 테스트 (Kotest)**
...
- **테스트 의존성 배치**: Kotest(`kotest-runner-junit5`, `kotest-assertions-core`)를 `oneulsogae-core`의 `testImplementation`에 둔다.
...
- **테스트 의존성 배치**: 통합/E2E 관련 의존성(Testcontainers, WireMock, Kotest, RestAssured)은 `oneulsogae-api`에 둔다.
  `IntegrationUtil`·픽스처는 `oneulsogae-infra`의 `testFixtures`로 제공하고, api가 `testImplementation(testFixtures(project(":oneulsogae-infra")))`로 가져온다.
