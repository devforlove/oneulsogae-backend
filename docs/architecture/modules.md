...
```
oneulsogae-api ──> oneulsogae-common, oneulsogae-core, oneulsogae-infra, oneulsogae-chatting, oneulsogae-scheduler, oneulsogae-auth  (유일한 Spring Boot 진입점, Controller/인증 + 배치 스케줄 구동)
oneulsogae-chatting ──> oneulsogae-common, oneulsogae-auth                             (WebSocket. core에 의존하지 않는 자립 모듈 — 자체 도메인/서비스/포트 보유)
oneulsogae-scheduler ──> oneulsogae-common                                         (배치 로직 + 자체 포트. core에 의존하지 않는 라이브러리)
oneulsogae-core ──> oneulsogae-common                                              (도메인/유스케이스/포트 = 비즈니스 핵심)
oneulsogae-infra ──> oneulsogae-common, oneulsogae-core, oneulsogae-scheduler, oneulsogae-chatting  (JPA 엔티티/Repository/Adapter. core·scheduler·chatting의 out-port를 구현)
oneulsogae-auth ──> (spring-security, jjwt)                                     (인증 검증 커널: TokenProvider/PrincipalDetails. 발급/로그인은 api)
oneulsogae-common ──> (없음)                                                    (공용 enum/상수)
```

- `oneulsogae-core`는 **순수 비즈니스 로직**이다. JPA·웹 프레임워크 등 인프라 세부에 의존하지 않는다.
- 영속성 구현은 `oneulsogae-infra`의 Adapter가 `oneulsogae-core`/`oneulsogae-scheduler`/`oneulsogae-chatting`의 **out-port를 구현**하는 방식으로 둔다.
- HTTP 경계(Controller / 요청·응답 DTO)는 `oneulsogae-api`에만 둔다.
- 공용 enum/상수는 `oneulsogae-common`에 둔다.

## 인증 검증 커널 분리 (oneulsogae-auth)

- JWT 검증·파싱과 인증 주체는 `oneulsogae-auth`에 둔다(`com.org.oneulsogae.auth.jwt.TokenProvider`, `com.org.oneulsogae.auth.PrincipalDetails`).
  `oneulsogae-auth`는 **발급/로그인/쿠키/SecurityConfig를 모르는** 순수 검증 커널이라, 토큰을 검증만 하면 되는 모듈(예: `oneulsogae-chatting`)이 api에 의존하지 않고 재사용할 수 있다.
- 발급·OAuth2 로그인·재발급·쿠키·`SecurityConfig`·MVC 바인딩 등 **인증 서버/게이트웨이 책임은 `oneulsogae-api`**에 남는다(이들이 `oneulsogae-auth`의 `TokenProvider`를 호출한다).

## 단일 부트 앱 (oneulsogae-api)

- `oneulsogae-api`가 **유일한 `@SpringBootApplication`**이며(`oneulsogae-scheduler`는 실행 가능한 부트 앱이 아닌 라이브러리),
  기준 패키지가 `com.org.oneulsogae`라 core 서비스·scheduler 배치 로직·infra 어댑터를 함께 스캔한다.
  (`@EnableScheduling`으로 스케줄러 빈도 활성화)

## 배치/스케줄 모듈 분리 (oneulsogae-scheduler)

- 배치 알고리즘·유스케이스·전용 포트/도메인은 `oneulsogae-scheduler`에 둔다.
  scheduler도 [CQRS 패키지 분리](hexagonal-cqrs.md)처럼 **`command`/`query` 패키지로 분리**한다
  (예: command `com.org.oneulsogae.scheduler.match.command.application.SoloMatchBatchService`,
...
- `oneulsogae-scheduler`는 **`oneulsogae-core`에 의존하지 않으므로**, 배치가 쓰는 매칭 이력 기록/조회·시각 등 공유 동작도
...
- `@Scheduled` **크론 트리거**는 `oneulsogae-api`의 `scheduler` 패키지(`com.org.oneulsogae.scheduler.match.SoloMatchBatchScheduler`)에
  `@Component`로 두고, scheduler 모듈의 실행 로직(`SoloMatchBatchJob` → `RunSoloMatchBatchUseCase`)을 호출한다.
...
## 채팅(WebSocket) 모듈 자립 (oneulsogae-chatting)

- `oneulsogae-chatting`은 **`oneulsogae-core`에 의존하지 않는다.** scheduler와 같은 자립 구조로,
  ...
  예: `com.org.oneulsogae.chatting.chat.domain.{ChatMessage, ChatRoom, ChatRoomMember(s)}`,
...
- 토큰 검증은 `oneulsogae-auth`의 `TokenProvider`로 하고(STOMP CONNECT 프레임), 핸드셰이크 경로(`/ws/chat/**`)는 `SecurityConfig`에서 permitAll로 열어 둔다.
