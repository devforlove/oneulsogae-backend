package com.org.oneulsogae.scheduler.solomatch.command.adapter
...
/**
 * 일일 매칭 배치 실행 로직.
 * [RunSoloMatchBatchUseCase]를 실행하고 시작/종료를 로깅한다.
 * "언제 돌릴지"(@Scheduled 크론 트리거 / 관리자 수동 트리거)는 이 모듈을 의존하는 실행 앱(oneulsogae-api)이 담당하고,
 * 이 클래스는 "무엇을 실행할지"(배치 실행 진입점)만 책임지는 애플리케이션 로직이다.
 * (배치 알고리즘 자체는 oneulsogae-core의 UseCase에 있다)
 *
