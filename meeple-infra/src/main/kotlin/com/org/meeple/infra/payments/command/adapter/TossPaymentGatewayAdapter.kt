 ...
 * 2xx(status DONE)면 승인 성공(true), 토스가 4xx/5xx로 거절하면 승인 실패(false).
 * 통신 자체가 실패(타임아웃 등)하면 예외를 그대로 전파한다 — 승인 여부가 불확실한데 false로 단정하면
 ...
 * ([StubPaymentGatewayAdapter](@Profile("!prod"))와 프로파일 대칭 — local/test는 스텁을 쓴다)
 */
@Component
@Profile("prod")
