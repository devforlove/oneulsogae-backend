 ...
 * prod는 실제 PG 어댑터([TossPaymentGatewayAdapter], @Profile("prod"))가 담당한다.
 */
@Component
@Profile("!prod")
class StubPaymentGatewayAdapter : PaymentGatewayPort {
	...
	override fun confirm(paymentKey: String, orderId: String, amount: Int): PaymentConfirmResult =
		if (confirmResult(stubHeader())) {
			PaymentConfirmResult(approved = true, failReason = null)
		} else {
			PaymentConfirmResult(approved = false, failReason = STUB_FAIL_REASON)
		}
	...
	companion object {
		...
		/** 스텁 승인 실패 시 기록하는 사유. (실 PG 응답 원문 자리) */
		const val STUB_FAIL_REASON: String = "STUB_PG_CONFIRM_FAILED"
