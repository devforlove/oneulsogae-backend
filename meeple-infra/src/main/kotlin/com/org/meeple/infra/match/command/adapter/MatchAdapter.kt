...
/**
 * [MatchEntity]의 command 영속성 어댑터. (매칭 엔티티당 command 어댑터는 이 하나 — core·scheduler 모듈의 command out-port를 함께 구현)
 ...
 * 조회 dao(core `MatchWithPartnerQueryDao`, scheduler `MatchRecordQueryDao`)는 query 패키지의 `*DaoImpl`이 별도로 구현한다.
 */
@Component
class MatchAdapter(
